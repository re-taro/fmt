...
let defaultConfig = ESLintUtils.deepMerge(
	{},
	testerDefaultConfig,
...
export class RuleTester extends TestFramework {
	...
	readonly #rules: Record<
		string,
		TSESLint.AnyRuleCreateFunction | TSESLint.AnyRuleModule
	> = {};

	readonly #linter = new TSESLint.Linter();
	...
	static setDefaultConfig(config: RuleTesterConfig): void {
		...
		defaultConfig = ESLintUtils.deepMerge(
			defaultConfig,
	...
	defineRule(
		name: string,
		rule: TSESLint.AnyRuleCreateFunction | TSESLint.AnyRuleModule,
	): void {
    ...
    */
		const getFilename = (testOptions?: TSESLint.ParserOptions): string => {
			const resolvedOptions = ESLintUtils.deepMerge(
    ...
    */
	...
	run<TMessageIds extends string, TOptions extends readonly unknown[]>(
		...
		this.#linter.defineRule(
			...
			Object.assign({}, rule, {
				...
				create(context: TSESLint.RuleContext<TMessageIds, TOptions>) {
	...
	>(
		...
		this.#linter.defineParser(
			config.parser,
			wrapParser(require(config.parser) as TSESLint.Parser.ParserModule),
		);
